...
define( 'PW_GALLERY_URL', get_bloginfo('stylesheet_directory').'/library/cmb-field-gallery-master/' );
...
function pw_gallery_field( $field, $meta ) {
	...
	if ( ! empty( $meta ) ) {
		echo '<div class="pw-gallery-preview">';
		echo pw_gallery_preview( $meta );
		echo '</div>';
		$meta = implode( ',', $meta );
	}
...
function pw_gallery_preview( $ids=array() ){
	foreach( $ids as $id ) {
		$return .= wp_get_attachment_image( $id, 'thumbnail', null, array(
			'style' => 'width:75px;height:auto;display:inline-block;margin-right:2px;'
		));
	}
	return $return;
}

function refresh_pw_gallery_preview(){

	if( empty( $_POST['ids'] ) ) {
		$html = '';
	} else {
		$html = pw_gallery_preview( $_POST['ids'] );
	}
	wp_send_json_success(array(
		'html' => $html
	));
}
add_action( 'wp_ajax_refresh_pw_gallery_preview', 'refresh_pw_gallery_preview' );